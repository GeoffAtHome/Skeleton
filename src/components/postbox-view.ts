...
export class PostboxView extends connect(store)(PageViewElement) {
  @query('#map')
  private map: any;
  ...
  private _postBoxKey: string = '';

  @property({ type: Object })
  private postBoxData: PostBoxList = {};
  ...
  updated(changedProps: PropertyValues) {
    if (changedProps.has('postBoxData')) this.drawLabels(this.postBoxData);
    if (changedProps.has('_postBoxKey') && this._postBoxKey !== '')
      this.displayPostBox(this._postBoxKey);
  }
  ...
  stateChanged(state: RootState) {
    if (this.active) {
      ...
      this.postBoxData = postboxState!._data;
      this._postBoxKey = postboxState!._postBoxKey;
    }
  }
  ...
  private displayPostBox(postBoxKey: string) {
    const postBox = this.postBoxData[postBoxKey];
